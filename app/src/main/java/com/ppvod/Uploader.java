        ...
        private void upload() {
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                try {
                    ...
                    code = root.getInt("ifExist");
                    LogUtil.debug("code is :" + code);
                    if (code == 1) {
                        LogUtil.info("分片" + curIdx + "已经存在,跳过!");
                        mTotalWrite += length;
                        curIdx++;
                        continue;
                    }
                    ...
                    if (chunkRetry >= MAX_RETRY) {
                        LogUtil.debug("分片失败次数已超过最大重试数,上传失败!" + chunkRetry);
                        mStatus = STATUS_ERROR;
                        break;
                    } else {
                        try {
                            LogUtil.debug("分片上传失败,准备重试" + chunkRetry);
                            Thread.sleep(5000);
